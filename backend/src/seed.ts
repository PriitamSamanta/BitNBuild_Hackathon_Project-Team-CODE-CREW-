...
const seedDatabase = async (): Promise<void> => {
    try {
        ...
        const fireIncident = await Incident.create({
            ...
            description:
                "Major fire reported inside a commercial building. Multiple people may be trapped.",
            severity: "critical",
            status: "reported",
            priority: 5,
            location: {
        ...
        const floodIncident = await Incident.create({
            ...
            description:
                "Heavy rainfall has caused water levels to rise rapidly in a residential area.",
            severity: "high",
            status: "verified",
            priority: 4,
            location: {
        ...
        const accidentIncident = await Incident.create({
            ...
            description:
                "Multiple vehicles involved in a road accident. Traffic is blocked and medical assistance is required.",
            severity: "high",
            status: "assigned",
            priority: 4,
            location: {
        ...
        const medicalIncident = await Incident.create({
            ...
            description:
                "Several people require urgent medical assistance following an incident.",
            severity: "medium",
            status: "reported",
            priority: 3,
            location: {
        ...
        const reports = await Report.insertMany([
            {
                reportId: "REP-0001",
                description:
                    "Large fire visible from Main Road. Smoke coming from the upper floors.",
                type: "fire",
                location: {
                    address: "Main Road, Vadodara",
                    latitude: 22.3072,
...

            {
                reportId: "REP-0002",
                description:
                    "Water level rising rapidly near residential buildings.",
                type: "flood",
                location: {
                    address: "Alkapuri, Vadodara",
                    latitude: 22.31,
        ...
        const alerts = await Alert.insertMany([
            {
                ...
                message:
                    "Immediate response required for Building Fire on Main Road.",
                incidentId: fireIncident._id,
                status: "active",
            },

            {
                ...
                message:
                    "Response to the flood incident has exceeded the expected response time.",
                incidentId: floodIncident._id,
                status: "active",
                metadata: {
                    delayMinutes: 7,
...

            {
                ...
                message:
                    "The building fire incident requires immediate escalation.",
                incidentId: fireIncident._id,
                status: "active",
                metadata: {
                    delayMinutes: 12,
