...
export const createCriticalIncidentAlert = async (incidentId) => {
...
export const checkDelayedResponses = async () => {
    const cutoff = new Date(Date.now() - RESPONSE_DELAY_MINUTES * 60 * 1000);
    ...
    for (const incident of incidents) {
        ...
        const delayMinutes = Math.floor((Date.now() - new Date(incident.updatedAt ?? Date.now()).getTime()) /
            60000);
        await createAlert({
            type: "response_delay",
            severity: "warning",
...
export const checkEscalations = async () => {
    const cutoff = new Date(Date.now() - ESCALATION_DELAY_MINUTES * 60 * 1000);
    const incidents = await Incident.find({
        status: { $in: ["assigned", "en_route"] },
        updatedAt: { $lte: cutoff },
    });
