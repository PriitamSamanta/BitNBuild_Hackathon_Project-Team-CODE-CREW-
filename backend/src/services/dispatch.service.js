...
export const dispatchTeams = async (incidentId, teamIds) => {
    ...
    const unavailableTeam = teams.find((team) => team.status !== "available");
    if (unavailableTeam) {
        throw new Error(`Team ${unavailableTeam.teamId} is not available`);
    }
    // Assign MongoDB team IDs to the incident.
    incident.assignedTeams = teams.map((team) => team._id);
    ...
    const updatedIncident = await Incident.findById(incident._id).populate("assignedTeams");
