...
const calculateTextSimilarity = (first, second) => {
...
export const findDuplicateIncidents = async (input) => {
    ...
    for (const incident of incidents) {
        const distanceKm = calculateDistanceKm(input.latitude, input.longitude, incident.location.latitude, incident.location.longitude);
        // Ignore incidents more than 2 km away.
        if (distanceKm > 2) {
            continue;
        }
        const textSimilarity = calculateTextSimilarity(input.description, `${incident.title} ${incident.description}`);
        let score = 0;
        // Location similarity
        if (distanceKm <= 0.5) {
            score += 0.5;
